// ═════════════════════════════════════════════════════════════════
// historial_reacciones.dart — Aspirantes ITVH
//
// Muestra todas las publicaciones a las que el usuario ha reaccionado,
// agrupadas por fecha (Hoy / Esta semana / Anteriores). Cada
// publicación se renderiza con TarjetaPublicacion (la misma tarjeta
// del feed); el ícono de comentar abre HojaComentarios y el contador
// de reacciones abre HojaReacciones, igual que en el feed.
//
// Clases principales:
//   • _TipoReaccion              — enum de tipos de reacción soportados
//   • _ItemReaccion              — modelo de datos de una reacción
//   • HistorialReaccionesScreen  — pantalla principal con lista agrupada
//
// Widgets privados:
//   • _EstadoVacio — placeholder cuando no hay reacciones
// ═════════════════════════════════════════════════════════════════
...
/// Tipos de reacción reconocidos por la app.
enum _TipoReaccion { like, love, funny, support, desconocido }

/// Representa una reacción del usuario junto con la publicación
/// completa lista para pasarle directo a [TarjetaPublicacion].
class _ItemReaccion {
  final String        id;
  final _TipoReaccion tipo;
  ...
  /// Convierte el string crudo de Supabase al enum [_TipoReaccion].
  static _TipoReaccion _parseTipo(String? raw) {
    switch (raw) {
      case 'like':    return _TipoReaccion.like;
      case 'love':    return _TipoReaccion.love;
      case 'funny':   return _TipoReaccion.funny;
      case 'support': return _TipoReaccion.support;
      default:        return _TipoReaccion.desconocido;
    }
  }

  /// Construye un [_ItemReaccion] a partir de la fila cruda de Supabase.
  ///
  ...
  factory _ItemReaccion.fromMap(Map<String, dynamic> map) {
    ...
    // El usuario ya reaccionó a esta publicación (por eso está en la lista).
    postMap['yo_di_like'] = true;

    return _ItemReaccion(
      id:            map['id'] as String,
...
class HistorialReaccionesScreen extends StatefulWidget {
  final bool isDark;
  const HistorialReaccionesScreen({super.key, required this.isDark});

  @override
  State<HistorialReaccionesScreen> createState() =>
      _HistorialReaccionesScreenState();
}

class _HistorialReaccionesScreenState
    extends State<HistorialReaccionesScreen> {
  List<_ItemReaccion> _items    = [];
  bool                _cargando = true;
  ...
  /// Consulta las 200 reacciones más recientes del usuario autenticado,
  ...
  Future<void> _cargar() async {
    ...
    try {
      ...
      final lista = (res as List)
          .cast<Map<String, dynamic>>()
          .map(_ItemReaccion.fromMap)
          .toList();

      if (mounted) setState(() { _items = lista; _cargando = false; });
    } catch (e) {
      debugPrint('HistorialReacciones – cargar: $e');
  ...
  /// Agrupa las reacciones en tres buckets temporales:
  ///   • Hoy         — menos de 24 horas
  ///   • Esta semana — entre 1 y 6 días
  ///   • Anteriores  — 7 días o más
  Map<String, List<_ItemReaccion>> _agruparPorFecha() {
    final hoy      = <_ItemReaccion>[];
    final semana   = <_ItemReaccion>[];
    final antiguas = <_ItemReaccion>[];
  ...
  /// Devuelve el emoji correspondiente a cada tipo de reacción.
  String _emojiTipo(_TipoReaccion tipo) {
    switch (tipo) {
      case _TipoReaccion.like:    return '❤️';
      case _TipoReaccion.love:    return '❤️';
      case _TipoReaccion.funny:   return '😂';
      case _TipoReaccion.support: return '🤝';
      default:                    return '👍';
    }
  }
  ...
  Future<void> _toggleReaccion(String publicacionId, bool nuevoEstado) async {
    ...
    try {
      ...
      debugPrint('HistorialReacciones – toggleReaccion: $e');
    }
  }

  void _abrirComentarios(_ItemReaccion item) {
  ...
  void _abrirReacciones(_ItemReaccion item) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: _cargando
          ...
          mensaje: 'No has reaccionado a ninguna publicación aún.',
        )
            : ListView(
              ...
              for (final item in entry.value)
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      // ── Etiqueta con emoji + tiempo ───────────
                      Text(
                        '${_emojiTipo(item.tipo)}  Reaccionaste a una publicación',
                        style: TextStyle(
                            color:      text1,
                            fontSize:   13,
                            fontWeight: FontWeight.w600),
